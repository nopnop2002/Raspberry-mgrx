...
//    "MGRX 1.0, the graphics library for DJGPPv2, Linux, X11 and Win32";
    "MGRX 1.0, the graphics library for RaspberryPi";
...
static Button bp1[NBUTTONSP1] = {
    ...
    {PX1, PY0, 100, 40, IND_BLUE, IND_YELLOW, "CircTest", 0, ID_CIRCTEST},
    {PX1, PY1, 100, 40, IND_BLUE, IND_YELLOW, "Colorops", 0, ID_COLOROPS},
    {PX1, PY2, 100, 40, IND_BLUE, IND_YELLOW, "CursTest", 0, ID_CURSTEST},
    {PX1, PY3, 100, 40, IND_BLUE, IND_YELLOW, "DemoIntl", 0, ID_DEMOINTL},
    {PX1, PY4, 100, 40, IND_BLUE, IND_YELLOW, "FontTest", BSTATUS_SELECTED, ID_FONTTEST},
    {PX2, PY0, 100, 40, IND_BLUE, IND_YELLOW, "FontDemo1", 0, ID_FNTDEMO1},
    {PX2, PY1, 100, 40, IND_GREEN, IND_WHITE, "Page 2", 0, ID_PAGE2},
    {PX2, PY2, 100, 40, IND_GREEN, IND_WHITE, "Page 3", 0, ID_PAGE3},
    {PX2, PY3, 100, 40, IND_BROWN, IND_WHITE, "ModeTest", 0, ID_MODETEST},
    {PX2, PY4, 100, 40, IND_RED, IND_WHITE, "Exit", 0, ID_EXIT}
};

#define NBUTTONSP2  14

static Button bp2[NBUTTONSP2] = {
    {PX0, PY0, 100, 40, IND_BLUE, IND_YELLOW, "FontDemo2", 0, ID_FNTDEMO2},
    {PX0, PY1, 100, 40, IND_BLUE, IND_YELLOW, "FontDemo3", 0, ID_FNTDEMO3},
    {PX0, PY2, 100, 40, IND_BLUE, IND_YELLOW, "FontDemo4", 0, ID_FNTDEMO4},
    {PX0, PY3, 100, 40, IND_BLUE, IND_YELLOW, "ImgTest", 0, ID_IMGTEST},
    {PX0, PY4, 100, 40, IND_BLUE, IND_YELLOW, "JpgTest", 0, ID_JPGTEST},
    {PX1, PY0, 100, 40, IND_BLUE, IND_YELLOW, "Keys", 0, ID_KEYS},
    {PX1, PY1, 100, 40, IND_BLUE, IND_YELLOW, "LineTest", 0, ID_LINETEST},
    ...
    {PX2, PY1, 100, 40, IND_GREEN, IND_WHITE, "Page 1", 0, ID_PAGE1},
    {PX2, PY2, 100, 40, IND_GREEN, IND_WHITE, "Page 3", 0, ID_PAGE3},
    {PX2, PY3, 100, 40, IND_BROWN, IND_WHITE, "ModeTest", 0, ID_MODETEST},
    {PX2, PY4, 100, 40, IND_RED, IND_WHITE, "Exit", 0, ID_EXIT}
};

#define NBUTTONSP3  14

static Button bp3[NBUTTONSP3] = {
    {PX0, PY0, 100, 40, IND_BLUE, IND_YELLOW, "PcircTst", 0, ID_PCIRCTST},
    {PX0, PY1, 100, 40, IND_BLUE, IND_YELLOW, "PngTest", 0, ID_PNGTEST},
    {PX0, PY2, 100, 40, IND_BLUE, IND_YELLOW, "PnmTest", 0, ID_PNMTEST},
    {PX0, PY3, 100, 40, IND_BLUE, IND_YELLOW, "PolyTest", 0, ID_POLYTEST},
    {PX0, PY4, 100, 40, IND_BLUE, IND_YELLOW, "ScrolTst", 0, ID_SCROLTST},
    ...
    {PX2, PY1, 100, 40, IND_GREEN, IND_WHITE, "Page 1", 0, ID_PAGE1},
    {PX2, PY2, 100, 40, IND_GREEN, IND_WHITE, "Page 2", 0, ID_PAGE2},
    {PX2, PY3, 100, 40, IND_BROWN, IND_WHITE, "ModeTest", 0, ID_MODETEST},
    {PX2, PY4, 100, 40, IND_RED, IND_WHITE, "Exit", 0, ID_EXIT}
};
static Button_Group bgp1 = { 05, 30, bp1, NBUTTONSP1, 0, 0 };
static Button_Group bgp2 = { 05, 30, bp2, NBUTTONSP2, 0, 0 };
static Button_Group bgp3 = { 05, 30, bp3, NBUTTONSP3, 0, 0 };
...
int main(int argc, char **argv)
{
    ...
    while (1) {
	GrEventRead(&ev);
//if (ev.type != 0) printf("main:ev.type=%d,ev.p1=%ld\n",ev.type,ev.p1);
...
if(_DEBUG_)printf("main:pev_button_group\n");
	    continue;
        }
	if (pev_command(&ev)) {
if(_DEBUG_)printf("main:pev_command\n");
	    continue;
        }
	if (pev_select(&ev)) {
if(_DEBUG_)printf("main:pev_select\n");
	    continue;
        }
	if (ev.type == GREV_MOUSE) {
if(_DEBUG_)printf("main:ev.type=GREV_MOUSE\n");
...
static void ini_graphics(void)
{
    ...
    gwidth = GrScreenX();
    gheight = GrScreenY();
if(_DEBUG_)printf("ini_graphics:gwidth=%d gheight=%d\n",gwidth,gheight);
    grcglob = NULL;
...
if(_DEBUG_)printf("pev_command:GREV_COMMAND ev->p1=%ld\n",ev->p1);
	if (ev->p1 == ID_EXIT) {
	...
	for (i = 0; i < NDEMOS; i++) {
if(_DEBUG_)printf("pev_command:ev->p1=%ld ptable[i].cid=%d\n",ev->p1,ptable[i].cid);
...
if(_DEBUG_)printf("pev_command:nprog=%s\n",nprog);
		system(nprog);
		ini_graphics();
                GrSetWindowTitle(wintitle);
		paint_screen();
